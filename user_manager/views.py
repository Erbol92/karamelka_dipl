...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        messages.success(request,
                         'Спасибо за подтверждение по электронной почте. Теперь вы можете войти в свою учетную запись.')
...
def profile(request):
    ...
    bdlc = profile_obj.b_date_last_change
    b_date = profile_obj.b_date
    form = ProfileForm(request.POST or None, request.FILES or None, instance=profile_obj, user=request.user)
    if form.is_valid():
        six_months_ago = datetime.now().date() - timedelta(days=182)
        obj = form.save(commit=True)
        print(bdlc, six_months_ago)
        if not bdlc or bdlc < six_months_ago:
            obj.b_date_last_change = datetime.now()
        else:
            obj.b_date_last_change=bdlc
            obj.b_date=b_date
            profile_obj.save()
            messages.info(request, 'нельзя менять дату рождения чаще полугода')
            return redirect('/profile')
        profile_obj.save()
